...
export const pgrep = {
  name: 'pgrep',
  description: 'Look up processes based on name and other attributes|根据名称和其他属性查找进程',
  ...
  handler(args, context, fs) {
    ...
    if (args.length === 0) {
      return formatHelp({
        name: 'pgrep',
        description: 'Look up processes based on name and other attributes|根据名称和其他属性查找进程',
        usage: 'pgrep [OPTIONS] PATTERN|pgrep [选项] 模式',
        options: [
          ...
          '-s, --session SID    Match session ID|匹配会话ID',
          ...
          '-d, --delimiter SEP  Use SEP as output delimiter|使用SEP作为输出分隔符'
        ],
        examples: [
          'pgrep firefox|查找所有firefox进程',
          'pgrep -u user vim|查找用户user的所有vim进程',
          'pgrep -f "python script.py"|查找匹配完整命令行的进程',
          'pgrep -c httpd|统计httpd进程数量',
          'pgrep -l chrome|列出chrome进程的PID和名称',
          'pgrep -n firefox|仅显示最新的firefox进程'
        ],
        notes: [
          'PATTERN can be a regular expression|模式可以是正则表达式',
          'Use pkill to kill matching processes|使用pkill终止匹配的进程',
          'Process information is simulated in this environment|在此环境中进程信息是模拟的'
        ]
      })
    }
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '-f' || arg === '--full') {
        ...
        return `pgrep: invalid option: ${arg}`
      } else {
        pattern = arg
        break
      }
    ...
    if (!pattern) {
      return 'pgrep: no pattern specified\nUsage: pgrep [OPTIONS] PATTERN'
    }
    ...
    const processes = [
      ...
      { pid: 4568, name: 'httpd', fullCmd: '/usr/sbin/httpd -D FOREGROUND', user: 'apache', uid: 48, gid: 48, ppid: 4567, session: 1, terminal: '?', age: 86300 },
    ...
    try {
      ...
      return `pgrep: invalid regular expression: ${pattern}`
    }
    ...
    if (matchingProcesses.length === 0) {
      return '' // pgrep returns empty output when no processes match
    }
    ...
    for (const proc of matchingProcesses) {
      if (options.listFull) {
        ...
        results.push(proc.pid.toString())
      }
    }
